...
def solve_puzzle(puzzle_input):
    ...
    for segment in line_segments:
        if segment.start.x == segment.end.x: # vertical line
        ...
        elif abs(segment.start.x-segment.end.x) == abs(segment.start.y-segment.end.y): # diagonal line
            ...
            if segment.start.y<segment.end.y:
                ...
                dy = -1
            for i in range(abs(segment.start.x-segment.end.x)+1):
                ocean_floor[Point(segment.start.x+dx*i,segment.start.y+dy*i)] += 1        
